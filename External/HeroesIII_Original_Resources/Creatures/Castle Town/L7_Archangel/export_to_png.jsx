...
function scanLayerSets(el) {
    ...
    for(var a=0;a<el.layerSets.length;a++){
        ...
        if (lname.substr(-4) == ".png") {
            saveLayer(el.layers.getByName(lname), el.name, lname, oldPath, true);
        } else {
            // recursive
    ...
    for(var j=0; j<el.artLayers.length; j++) {
        ...
        ////if (name.substr(-4) == ".png") {
            ...
            saveLayer(el.layers.getByName(name), el.name, name, oldPath, false);
        ////}
    }

}

function saveLayer(layer, groupName, lname, path, shouldMerge) {
    ...
    var docName = activeDocument.name + "_" + groupName + "_" + lname + ".png"
    var saveFile= File(path + "/out/" + docName);
    SavePNG(saveFile);
