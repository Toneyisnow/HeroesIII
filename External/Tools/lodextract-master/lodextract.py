...
def unpack_lod(infile,outdir):
    ...
    for i in range(total):
        ...
        print "reading from cursor:", str(cursor)
        ...
        offset,size,_,csize = struct.unpack("<IIII", f.read(16))
        print "filename:", filename, " offset:" + str(offset) + " size:" + str(size) + " csize:" + str(csize)
        cursor += 16
        files.append((filename,offset,size,csize))

    for filename,offset,size,csize in files:
        filename=os.path.join(outdir,filename)
        print "unpack file:", filename
        f.seek(offset)
        data = f.read(csize)
        print "file offset: ", str(offset), " csize:", str(csize), " length of the file: ", len(data)
